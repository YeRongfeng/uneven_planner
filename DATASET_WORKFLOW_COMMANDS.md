...
3. 顶部先选地形目录，再选具体原始地图。
4. 中间画布框选约 20 m 区域。右侧看二维、3D 和「拟合地形」。拟合按 YRF：体素 0.2 m、粗格 0.4 m、三次插值到 0.2 m；低于裁块中位数 30 m 的超低飞点会先丢掉。
5. 选训练集或验证集，点「通过并保存」或「拒绝并保存」。也可用「通过/拒绝并随机下一块」。
6. 继续旧工作时，勾选显示全部历史选区，或从历史列表点回去看。

### B. 生成中间地图

7. 「生成」页点「生成训练用地图」。这会按通过记录裁出 20 m 中间图（`dataset/canonical/<任务名>/`）。
8. 若只是打回后补了一张，勾选「仅生成已补位的地图」，只重写那个编号（例如 `val/map_025` 就是 `map_025`），不会重建全部。

### C. 测试生成（筛掉出不了轨迹的图）

9. 先点「测试生成（每张 1 条）」，不要先点正式生成。
10. 每张图只出 1 条轨迹。一对起终点失败会换下一对；换满 30 对还是 0 条，才标「需要打回」并跳过，同时自动写进 8767 的待补位。已经有 1 条或已经标打回的图不会重测。8765 也可以勾选后批量打回。
11. 测试结果写在最终数据目录里，目录因此会非空。随后点「生成最终训练数据」不会因为目录已有内容而失败；测试出的那 1 条会留下来，正式生成在上面补齐。

### D. 查看、打回、补位

12. 「生成」页「启动结果查看器」→「打开结果查看器」（<http://127.0.0.1:8765/>）。8765 徽章是「需要打回」或「正常」，不再用 easy/medium/hard 评级当主标签。
13. 需要换掉的图点「打回，去8767补位」。该编号出现在 8767 的「补充空位」里。
14. 回到 8767 重新框选，在「补充空位」里选「补充 train」或「补充 val」（不用再点具体编号，会自动占用该集里最小的空位），然后保存通过。选一次后可以连着补同一边。
15. 勾选「仅生成已补位」，再生成中间图；需要的话再测一次补上的图。

### E. 正式生成轨迹

16. 确认没有待补空位后，点「生成最终训练数据」。这时不再打回。
17. 已标打回的图会跳过；测试时已经有 1 条的图会接着把剩余条数补齐。训练/验证每张图要多少条，在「调整生成数量或保存位置」里改。
18. 若只补了几张打回图，可勾选「仅生成已补位地图的轨迹」，只给那些编号出轨迹。
19. 看完在 8767 点「关闭结果查看器」。最终数据在 `dataset/public_terrain_20m/<任务名>/`，包括 `train/`、`val/`、`map.p` 和 `path_*.p`。
