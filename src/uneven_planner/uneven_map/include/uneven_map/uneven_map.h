...
namespace uneven_planner
{
    ...
    class UnevenMap
    {
        private:
            ...
            // external occupancy subscription
            bool use_external_occ_subscriber = false;    // if true, don't build occ map internally
            ros::Subscriber occ_sub;                     // subscriber for external occupancy map (now expects Float32MultiArray HWY)
        ...
        public:
            ...
            // callback to receive external occupancy map (expects 3D HWY Float32MultiArray)
            void occMapCallback(const std_msgs::Float32MultiArrayConstPtr& msg);
