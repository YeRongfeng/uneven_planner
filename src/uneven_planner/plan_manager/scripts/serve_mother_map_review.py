    ...
    def fit(self, center_x, center_y, size, yaw_deg):
        ...
        return {
            ...
            "method": (
                ...
                f"{resolution:.2f}m / 100x100 output"),
...
function nextOpenSlot(split){return (OPEN_SLOTS||[]).filter(item=>item.split===split).sort((a,b)=>a.map_index-b.map_index)[0]||null}
function renderOpenSlots(){const host=$('openSlots'),select=$('fillSlot'),slots=OPEN_SLOTS||[],train=slots.filter(item=>item.split==='train').length,val=slots.filter(item=>item.split==='val').length;if(host)host.innerHTML=slots.length?`<strong>待补位 ${slots.length} 个</strong><div>train ${train} · val ${val}</div><div>${slots.map(slot=>slot.key).join(' · ')}</div>`:'<span>当前没有待补位地图</span>';if(!select)return;select.options[0].text=slots.length?'不补充':'没有待补位';select.options[1].text=train?`补充 train（下一个 ${nextOpenSlot('train').key}）`:'补充 train（没有空位）';select.options[1].disabled=!train;select.options[2].text=val?`补充 val（下一个 ${nextOpenSlot('val').key}）`:'补充 val（没有空位）';select.options[2].disabled=!val;if(state.fillSlotKey&&!nextOpenSlot(state.fillSlotKey))state.fillSlotKey='';select.value=state.fillSlotKey;const upcoming=nextOpenSlot(state.fillSlotKey);$('fillSlotHint').textContent=upcoming?`通过后占用 ${upcoming.key}，下一次仍补这一边。`:(slots.length?'选补充 train 或 val，通过后自动占用该集里编号最小的空位。':'选择候选区域后可直接新增审核记录')}
...
function fitSummaryHtml(){if(!state.fit)return'';const s=state.fit.stats,voxel=s.voxel_size_m??0.1,coarse=s.coarse_resolution_m??s.surface_cell_size_m??0.2,grid=s.grid_resolution_m??0.2,flyers=s.low_flyer_removed||0,flyerLine=flyers?`<div>滤除低于裁块中位数 ${(s.low_flyer_below_m??30).toFixed(0)} m 的超低噪声 ${flyers} 点</div>`:'';return`<div class="fit-summary"><div>候选区原始点 ${s.raw_points_in_fit_margin.toLocaleString()} 点，参与拟合 ${s.fit_points.toLocaleString()} 点</div><div>${s.classification_policy}</div><div>体素 ${voxel.toFixed(2)} m · 粗网格 ${coarse.toFixed(2)} m 取最高点直接作为输出 ${grid.toFixed(2)} m（100×100）</div>${flyerLine}<div>高程网格观测 ${(s.observed_fraction*100).toFixed(1)}% · 高程范围 ${s.elevation_range_m[0].toFixed(2)}–${s.elevation_range_m[1].toFixed(2)} m</div><div>坡度 median ${s.slope_degrees.median.toFixed(1)}° / p95 ${s.slope_degrees.p95.toFixed(1)}° / max ${s.slope_degrees.maximum.toFixed(1)}°</div></div>`}
...
function datasetBody(test){const manifest=$('manifestPath').value.trim()||`${$('canonicalDir').value.trim()}/approved_canonical_manifest.json`;return{manifest_path:manifest,output_dir:$('datasetDir').value.trim(),train_paths:test?1:+$('trainPaths').value,val_paths:test?1:+$('valPaths').value,workers:+$('workers').value,ros_port:+$('rosPort').value,only_filled_slots:test?$('onlyFilledTest').checked:$('onlyFilledDataset').checked,test_generation:Boolean(test)}}
