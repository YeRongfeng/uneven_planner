...
class GridTransformer:
    ...
    def __init__(self, coarse_resolution=0.4, fine_resolution=0.2, voxel_size=0.2):
        """
        初始化GridTransformer，完全匹配grid_transformer.cpp的参数
        
        Args:
            coarse_resolution: 粗糙网格分辨率（grid_coarse_resolution_ = 0.4）
            fine_resolution: 精细网格分辨率（grid_desired_resolution_ = 0.2）
            voxel_size: 体素降采样大小（voxelSize = 0.2）
        """
        self.coarse_resolution = coarse_resolution  # 对应grid_coarse_resolution_
        self.fine_resolution = fine_resolution      # 对应grid_desired_resolution_
        self.voxel_size = voxel_size               # 对应voxelSize
        self.search_radius = voxel_size * 5.0      # 对应ne.setRadiusSearch(voxelSize*5)
        ...
        rospy.loginfo(f"GridTransformer initialized: coarse_res={coarse_resolution}, fine_res={fine_resolution}, voxel_size={voxel_size}")

    def preprocess_pointcloud(self, pcd):
        """点云预处理：降采样和法向量计算，完全匹配C++逻辑"""
        rospy.loginfo("Starting point cloud preprocessing with C++ parameters...")

        # 体素降采样（完全匹配C++的voxelSize = 0.2）
        ...
        # 计算法向量（使用稍大的搜索半径来获得更平滑的法向量）
        # 原始PCL参数：voxelSize*5 = 0.2*5 = 1.0
        # 为了匹配话题数据的平滑性，我们尝试稍大的半径
        smooth_radius = self.search_radius * 1.2  # 增加20%的搜索半径
        downsampled_pcd.estimate_normals(
            search_param=o3d.geometry.KDTreeSearchParamRadius(radius=smooth_radius),
            fast_normal_computation=False  # 使用精确计算，避免快速近似
        )

        # 精确模拟PCL的setViewPoint(0,0,0)行为
        # PCL的viewpoint影响法向量的方向选择
        normals_array = np.asarray(downsampled_pcd.normals)
        points_array = np.asarray(downsampled_pcd.points)
        
        # PCL的setViewPoint(0,0,0)会确保法向量指向视点方向
        # 这意味着从原点看向表面，法向量应该指向观察者
        ...
        for i in range(len(points_array)):
            ...
            view_vector_norm = np.linalg.norm(view_vector)
            
            if view_vector_norm > 1e-8:
                view_vector = view_vector / view_vector_norm
                
                # PCL的setViewPoint确保法向量与视线向量夹角小于90度
                # 即法向量倾向于指向表面的"外侧"（朝向视点）
                if np.dot(normal, view_vector) < 0:
                    normals_array[i] = -normal

        rospy.loginfo("Point cloud preprocessing completed with PCL-style viewpoint correction")
    ...
    # def process_grid_cells(self, points, normals, grid_dict, map_size):
    #     """处理栅格单元，完全匹配grid_transformer.cpp的逻辑"""
    #     width, height = map_size
    ...
    #     rospy.loginfo(f"Processing {len(grid_dict)} grid cells using C++ logic...")

    #     for (grid_x, grid_y), point_indices in grid_dict.items():
    #         if len(point_indices) < 1:
    #             # 与grid_transformer.cpp保持一致：空单元设置为NaN
    #             elevation_grid[grid_y, grid_x] = np.nan
    #             normal_x_grid[grid_y, grid_x] = np.nan
    #             normal_y_grid[grid_y, grid_x] = np.nan
    #             normal_z_grid[grid_y, grid_x] = np.nan
    #             continue

    #         # 完全匹配C++逻辑：找到最高点和min_z点
    #         min_z = np.inf
    #         highest_elevation = -np.inf
    #         highest_index = 0
    #         min_z_index = 0

    #         # 遍历所有点，找到最高点
    #         for idx, point_idx in enumerate(point_indices):
    #             if points[point_idx][2] > highest_elevation:
    #                 highest_elevation = points[point_idx][2]
    #                 highest_index = idx

    #             # 计算法向量的标准化z分量绝对值（使用全局索引）
    #             normal = normals[point_idx].astype(np.float32)  # 确保数据类型一致
    #             x, y, z = float(normal[0]), float(normal[1]), float(normal[2])  # 转换为float
    #             norm_length = np.sqrt(x*x + y*y + z*z)  # 完全匹配C++的计算方式
                
    #             if norm_length > 1e-8:
    #                 # 完全匹配C++逻辑: abs(z/sqrt(x*x+y*y+z*z))
    #                 normalized_z_abs = abs(z / norm_length)  # 直接除法，匹配C++
    #                 # normalized_z_abs = (z / norm_length)  # 直接除法，匹配C++
    #                 if normalized_z_abs <= min_z:
    #                     min_z = normalized_z_abs
    #                     min_z_index = point_idx  # 修正：保存全局索引

    #         # 使用最高点的高程
    #         elevation_grid[grid_y, grid_x] = highest_elevation

    #         # 使用min_z_index点的法向量
    #         selected_normal = normals[min_z_index].astype(np.float32)
    #         x, y, z = float(selected_normal[0]), float(selected_normal[1]), float(selected_normal[2])
    #         norm_length = np.sqrt(x*x + y*y + z*z)  # 完全匹配C++: sqrt(x*x+y*y+z*z)
            
    #         if norm_length > 1e-8:
    #             # 完全匹配C++的归一化逻辑，但需要检查是否需要负号
    #             # 根据测试结果决定是否需要负号来匹配ROS坐标系
    #             normal_x_grid[grid_y, grid_x] = (x / norm_length)  # 保持负号，因为测试显示这样能得到正相关
    #             normal_y_grid[grid_y, grid_x] = (y / norm_length)  # 保持负号
    #             normal_z_grid[grid_y, grid_x] = (z / norm_length)  # 保持负号
    #         else:
    #             # 处理零向量的情况
    #             normal_x_grid[grid_y, grid_x] = np.nan
    #             normal_y_grid[grid_y, grid_x] = np.nan
    ...
    #     rospy.loginfo("Grid cell processing completed using C++ logic")
    #     return elevation_grid, normal_x_grid, normal_y_grid, normal_z_grid
