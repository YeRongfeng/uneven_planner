...
function draw3d(){const {w,h}=setupCanvas(),s=current(),d=decodeScene(s),faces=[],zmid=(s.elevation_min+s.elevation_max)/2,exag=+$('exag').value,[lo,hi]=layerRange(s);ctx.fillStyle='#0d1114';ctx.fillRect(0,0,w,h);function corner(col,row,z){return project(col/s.width*2-1,row/s.height*2-1,z,w,h)}for(let r=0;r<s.height;r++)for(let c=0;c<s.width;c++){const i=r*s.width+c;if(!d.v[i])continue;const z=(valueAt(s,d,'elevation',i)-zmid)/20*exag*2,q=[{p:corner(c,r,z)},{p:corner(c+1,r,z)},{p:corner(c+1,r+1,z)},{p:corner(c,r+1,z)}];faces.push({q,depth:q.reduce((a,v)=>a+v.p[2],0)/4,t:(valueAt(s,d,state.layer,i)-lo)/(hi-lo||1)})}faces.sort((a,b)=>b.depth-a.depth);for(const f of faces){const color=palette(f.t,state.layer,true);ctx.beginPath();ctx.moveTo(f.q[0].p[0],f.q[0].p[1]);for(let i=1;i<4;i++)ctx.lineTo(f.q[i].p[0],f.q[i].p[1]);ctx.closePath();ctx.fillStyle=`rgb(${color[0]},${color[1]},${color[2]})`;ctx.fill();if($('grid').checked){ctx.strokeStyle='#07101555';ctx.lineWidth=.5;ctx.stroke()}}drawTrajectories3d(s,d,w,h,zmid,exag);ctx.fillStyle='#b5c0c7';ctx.font='12px system-ui';ctx.fillText(`20 m × 20 m · ${s.width}×${s.height} 格 · 高差 ${fmt(s.elevation_max-s.elevation_min)} m · 垂直夸张 ${exag}×`,14,22);drawLegend(w,h,s)}
