...
function renderList(){const root=$('sceneList');root.textContent='';for(const i of state.filtered){const s=SCENES[i],el=document.createElement('div');el.className='scene-card'+(i===state.selected?' active':'');el.innerHTML=`<div class="scene-name">${s.display_id||s.id}</div><span class="badge ${returnStatus(s)}">${returnLabel(s)}</span><div class="scene-sub">${artifactLabel(s)} · ${trajectoryCount(s)} 条轨迹</div>`;el.onclick=()=>select(i);root.appendChild(el)}root.querySelector('.active')?.scrollIntoView({block:'nearest'})}
...
function updateReplacementButton(){const button=$('replaceScene'),status=$('replaceStatus'),s=current();if(!button)return;if(!LIVE){button.disabled=true;status.textContent='静态查看器不可替换';return}button.disabled=!s||Boolean(state.replacementJob);status.textContent=state.replacementJob?(state.replacementJob.status==='completed'?'已打回，等待8767补位':'处理中…'):s?(s.needs_return?'这张图出不了轨迹，请打回补位':'打回后到8767补充该空位'):'选择地图后可用'}
