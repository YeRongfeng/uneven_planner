...
const state={selected:0,filtered:[],mode:'2d',layer:'elevation',trajectory:'0',view2d:{zoom:1,px:0,py:0},view3d:{az:-0.72,el:0.68,zoom:1},drag:null,cache:new Map(),replacementJob:null};
...
function updateFilters(){const text=$('search').value.trim().toLowerCase(),domain=$('domainFilter').value,grades=new Set([...$('gradeFilters').querySelectorAll('input:checked')].map(x=>x.value));state.filtered=SCENES.map((s,i)=>[s,i]).filter(([s])=>(domain==='all'||(s.domain||s.metadata.domain)===domain)&&grades.has(s.grade)&&(!text||`${s.display_id||s.id} ${s.metadata.site_id||''} ${s.grade}`.toLowerCase().includes(text))).sort((a,b)=>{const mode=$('sort').value;if(mode==='scoreAsc')return a[0].score-b[0].score;if(mode==='scoreDesc')return b[0].score-a[0].score;if(mode==='yaw')return (a[0].metadata.crop_yaw_deg||0)-(b[0].metadata.crop_yaw_deg||0);return (a[0].display_id||a[0].id).localeCompare(b[0].display_id||b[0].id)}).map(x=>x[1]);renderList();$('summary').textContent=`${state.filtered.length}/${SCENES.length} 个场景`;draw()}
...
function renderList(){const root=$('sceneList');root.textContent='';for(const i of state.filtered){const s=SCENES[i],el=document.createElement('div');el.className='scene-card'+(i===state.selected?' active':'');el.innerHTML=`<div class="scene-name">${s.display_id||s.id}</div><span class="badge ${s.grade}">${s.grade}</span><div class="scene-sub">${artifactLabel(s)} · ${trajectoryCount(s)} 条轨迹 · score ${fmt(s.score,1)}</div>`;el.onclick=()=>select(i);root.appendChild(el)}root.querySelector('.active')?.scrollIntoView({block:'nearest'})}
...
function updateReplacementButton(){const button=$('replaceScene'),status=$('replaceStatus'),s=current();if(!button)return;if(!LIVE){button.disabled=true;status.textContent='静态查看器不可替换';return}button.disabled=!s||Boolean(state.replacementJob);status.textContent=state.replacementJob?(state.replacementJob.status==='completed'?'已打回，等待8767补位':'处理中…'):s?'打回后到8767补充该空位':'选择地图后可用'}
function renderDetails(){updateReplacementButton();const s=current(),m=s.metadata,q=s.quality_metrics;$('detailName').textContent=s.display_id||s.id;$('detailSource').textContent=`${s.domain||m.domain||''} · ${m.site_id||''} · ${s.relative_source}`;const cards=[['类型',artifactLabel(s)],['数据分割',s.split],['轨迹数',trajectoryCount(s)],['等级',s.grade],['几何分数',fmt(s.score,1)],['最终有效率',`${fmt((q.valid_fraction??m.valid_fraction)*100,2)}%`],['直接支撑率',`${fmt((q.observed_fraction??m.observed_fraction)*100,2)}%`],['物理障碍格',m.obstacle_cells_in_patch??'—'],['高差',`${fmt(s.elevation_max-s.elevation_min,3)} m`],['坡度 P95',`${fmt(q.slope_p95_deg??m.slope_degrees?.p95,2)}°`]];$('stats').innerHTML=cards.map(([k,v])=>`<div class="stat"><div class="k">${k}</div><div class="v">${v}</div></div>`).join('');const rows=[['地形类别',s.domain||m.domain],['环境',m.environment_id],['母图站点',m.site_id],['中心 X/Y',m.source_center_xy?.map(x=>fmt(x,2)).join(', ')],['CRS',m.source_crs],['1 m 源支撑',`${fmt((m.source_support_coverage||0)*100,2)}%`],['原始点数',m.source_raw_points_in_patch],['障碍物点数',m.obstacle_points_in_patch],['拟合半径',`${m.processing?.fit_radius_m??'—'} m`],['缺口补全',m.processing?.gap_completion],['许可',m.license]];$('metadata').innerHTML=`<table>${rows.map(([k,v])=>`<tr><td>${k}</td><td>${v??'—'}</td></tr>`).join('')}</table><details><summary>完整元数据</summary><pre></pre></details>`;$('metadata').querySelector('pre').textContent=JSON.stringify({quality:{grade:s.grade,score:s.score,metrics:q},metadata:m},null,2)}
...
async function pollReplacement(jobId){try{const job=await responseJson(await fetch(`/api/replacement-status?id=${encodeURIComponent(jobId)}`));if(!state.replacementJob||state.replacementJob.id!==jobId)return;if(job.status==='running'||job.status==='starting'){state.replacementJob=job;updateReplacementButton();setTimeout(()=>pollReplacement(jobId),1500);return}state.replacementJob=job;updateReplacementButton();if(job.status==='completed'){await refreshLive();$('replaceStatus').textContent='已打回，8767 已出现待补位';}else{$('replaceStatus').textContent=`打回失败：${job.error||'请查看任务日志'}`}}catch(error){state.replacementJob=null;updateReplacementButton();$('replaceStatus').textContent=`打回状态读取失败：${error.message}`}}
async function replaceSelectedScene(){const s=current();if(!LIVE||!s||state.replacementJob)return;if(!confirm(`确定打回 ${s.display_id||s.id} 吗？该地图编号会在 8767 显示为待补位，人工通过新区域后再重新生成 canonical。`))return;state.replacementJob={id:'starting',status:'starting'};updateReplacementButton();$('replaceStatus').textContent='正在登记待补位…';try{const job=await postJson('/api/replace-scene',{id:s.id});state.replacementJob=job;updateReplacementButton();pollReplacement(job.id)}catch(error){state.replacementJob=null;updateReplacementButton();$('replaceStatus').textContent=`登记失败：${error.message}`}}
