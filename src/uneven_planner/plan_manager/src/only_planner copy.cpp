...
namespace uneven_planner
{
    ...
    void OnlyPlanner::rcvWpsCallBack(const geometry_msgs::PoseStampedConstPtr& msg)
    {
        ...
        // 定义地图边界常量
        const double MAP_BOUNDARY = 4.9999;  // 与地图边界一致
        ...
        // 检查航点数量，如果过多则直接标记为失败
        const size_t SAFE_WAYPOINT_LIMIT = 40;
        ...
        if (enable_optimization) {
            ...
            // 对优化后的轨迹进行重采样
            ROS_INFO("Resampling optimized trajectory to %d points for consistent visualization", resample_points);
            traj_msg = createResampledTrajectoryMsg(back_end_traj);
        } else {
            // 轨迹优化被禁用，直接对A*路径进行重采样
            ROS_WARN("Trajectory optimization disabled, resampling A* path to %d points", resample_points);
    ...
    void OnlyPlanner::generateRandomPoses(Eigen::Vector3d& start_pose, Eigen::Vector3d& target_pose) {
        // 地图边界参数（与launch文件一致）
        const double MAP_X_MIN = -4.4;
        const double MAP_X_MAX = 4.4;
        const double MAP_Y_MIN = -4.4;
        const double MAP_Y_MAX = 4.4;
    ...
    mpc_controller::SE2Traj OnlyPlanner::createResampledPathMsg(const std::vector<Eigen::Vector3d>& path,
        ...
        // ROS_INFO("Corrected path endpoints: start=[%.3f, %.3f, %.3f], end=[%.3f, %.3f, %.3f]",
        //          corrected_path[0].x(), corrected_path[0].y(), corrected_path[0].z(),
        //          corrected_path.back().x(), corrected_path.back().y(), corrected_path.back().z());
        ...
        for (size_t i = 1; i < corrected_path.size(); i++) {
            double segment_length = (corrected_path[i].head<2>() - corrected_path[i-1].head<2>()).norm();
            total_length += segment_length;
            cumulative_lengths.push_back(total_length);
        }

        // 重采样到固定数量的点
        double dt = total_time / (resample_points - 1); 

        // ROS_INFO("Resampling path: original_points=%zu, cleaned_points=%zu, total_length=%.3f, target_points=%d",
        //          path.size(), corrected_path.size(), total_length, resample_points);
        ...
        for (int i = 0; i < resample_points; i++) {
            ...
            if (i == 0) {
                // 第一个点：起始点
                interpolated_point = corrected_path[0];
            } else if (i == resample_points - 1) {
                // 最后一个点：精确的目标点
                interpolated_point = target_state;
            } else {
                // 中间点：按长度比例插值
                double target_length = (double)i / (double)(resample_points - 1) * total_length;
                // 线性插值
                size_t idx = 0;
                ...
                if (idx < corrected_path.size() - 1 && cumulative_lengths[idx + 1] > cumulative_lengths[idx]) {
                    double ratio = (target_length - cumulative_lengths[idx]) /
                                  (cumulative_lengths[idx + 1] - cumulative_lengths[idx]);
                    ratio = std::max(0.0, std::min(1.0, ratio));  // 限制比例在[0,1]
                    interpolated_point = corrected_path[idx] + ratio * (corrected_path[idx + 1] - corrected_path[idx]);
                } else {
                    interpolated_point = corrected_path[std::min(idx, corrected_path.size() - 1)];
                }
            }

            // 边界检查和约束（与A*规划器的isInMap函数一致）
            const double RESAMPLED_MAP_BOUNDARY = 4.3999;  // 重采样时的边界检查
            if (interpolated_point(0) < -RESAMPLED_MAP_BOUNDARY || interpolated_point(0) > RESAMPLED_MAP_BOUNDARY ||
                interpolated_point(1) < -RESAMPLED_MAP_BOUNDARY || interpolated_point(1) > RESAMPLED_MAP_BOUNDARY) {
                ROS_WARN("Point %d out of map bounds: [%.3f, %.3f], clamping to boundary", i,
                         interpolated_point(0), interpolated_point(1));

                handlePlanningFailure();
                return traj_msg;  // 直接返回空轨迹消息              

                // // 约束到地图边界内
                // interpolated_point(0) = std::max(-RESAMPLED_MAP_BOUNDARY, std::min(RESAMPLED_MAP_BOUNDARY, interpolated_point(0)));
                // interpolated_point(1) = std::max(-RESAMPLED_MAP_BOUNDARY, std::min(RESAMPLED_MAP_BOUNDARY, interpolated_point(1)));
            }

            // 添加位置点
            geometry_msgs::Point pos_pt;
            pos_pt.x = interpolated_point(0);
            pos_pt.y = interpolated_point(1);
            pos_pt.z = 0.0;
            traj_msg.pos_pts.push_back(pos_pt);

            // 添加偏航角点
            geometry_msgs::Point angle_pt;
            angle_pt.x = interpolated_point(2);
            angle_pt.y = 0.0;
            angle_pt.z = 0.0;
            traj_msg.angle_pts.push_back(angle_pt);
        ...
        // ROS_INFO("Resampled A* path from %zu points to %d points", path.size(), resample_points);
        return traj_msg;
    }
    ...
    mpc_controller::SE2Traj OnlyPlanner::createResampledTrajectoryMsg(const SE2Trajectory& traj) {
        ...
        // ROS_INFO("Resampling trajectory: duration=%.3f, target_points=%d",
        //          total_duration, resample_points);

        // 均匀重采样位置轨迹
        for (int i = 0; i < resample_points; i++) {
            double t = (double)i / (double)(resample_points - 1) * total_duration;
        ...
        // 设置时间信息（均匀分布）
        double dt = total_duration / (resample_points - 1);
        for (int i = 0; i < resample_points - 1; i++) {
            traj_msg.posT_pts.push_back(dt);
            traj_msg.angleT_pts.push_back(dt);
        }

        // 设置初始速度和加速度（数值差分）
        Eigen::Vector2d pos_0 = traj.pos_traj.getValue(0.0);
        Eigen::Vector2d pos_dt = traj.pos_traj.getValue(0.01);  // 小时间步长
        Eigen::Vector2d init_vel = (pos_dt - pos_0) / 0.01;

        // 设置零加速度
        Eigen::Vector2d init_acc = Eigen::Vector2d::Zero();

        traj_msg.init_v.x = init_vel(0);
        traj_msg.init_v.y = init_vel(1);
        traj_msg.init_v.z = 0.0;

        traj_msg.init_a.x = init_acc(0);
        traj_msg.init_a.y = init_acc(1);
        traj_msg.init_a.z = 0.0;

        // ROS_INFO("Created resampled trajectory with %zu position points and %zu angle points",
        //          traj_msg.pos_pts.size(), traj_msg.angle_pts.size());
